...
@section('content')
    <style>
        ...
        .run-left::-webkit-scrollbar,
        .run-right::-webkit-scrollbar { width: 7px; }
        .run-left::-webkit-scrollbar-track,
        .run-right::-webkit-scrollbar-track { background: rgba(255,255,255,0.03); }
        .run-left::-webkit-scrollbar-thumb,
        .run-right::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.18); border-radius: 6px; }
        .run-left::-webkit-scrollbar-thumb:hover,
        .run-right::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.32); }
        ...
        .run-nav .nav-btn-prev {
            color: #94a3b8;
            border-color: rgba(255,255,255,0.18);
        }

        .run-nav .nav-btn-prev:hover {
            color: #e2e8f0;
            background: rgba(255,255,255,0.06);
            border-color: rgba(255,255,255,0.28);
        }

        .run-nav .nav-btn-next {
            color: #cbd5e1;
        ...
        @media (max-width: 900px) {
            .run-layout { grid-template-columns: 1fr; overflow-y: auto; }
            .run-left { border-right: none; border-bottom: 1px solid rgba(255,255,255,0.06); }
        }
    ...
    <div class="run-header">
        <h1 style="font-size:1.05rem;font-weight:800;color:#fff;letter-spacing:0.06em;text-transform:uppercase;">Run</h1>
    ...
    <div id="help-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:1000;align-items:center;justify-content:center;" onclick="if(event.target===this)this.style.display='none'">
        <div style="background:#0d1f3a;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:1.5rem 1.75rem;max-width:460px;width:90%;position:relative;">
            ...
            <h3 style="font-size:0.95rem;font-weight:700;color:#fff;margin-bottom:0.75rem;display:flex;align-items:center;gap:0.5rem;">
                <span style="background:rgba(37,99,235,0.2);color:var(--blue-400);padding:0.2rem 0.7rem;border-radius:6px;font-size:0.8rem;">RUN</span>
                Petunjuk Tahap
            </h3>
            <ul style="font-size:0.85rem;color:#94a3b8;line-height:1.8;padding-left:1.2rem;">
                <li>Klik tombol <strong style="color:#cbd5e1;">Run ▶</strong> untuk menjalankan kode SQL</li>
                <li>Perhatikan output tabel yang muncul</li>
                <li>Bandingkan dengan prediksimu di tahap sebelumnya</li>
                <li>Jawab pertanyaan refleksi di kolom jawaban, lalu klik <strong style="color:#cbd5e1;">Submit</strong></li>
            </ul>
        </div>
    </div>
    ...
    <div class="run-layout">
        ...
        <div class="run-left">
            ...
            {{-- Prediksi Anda --}}
            @if ($predictSubmission)
                <div class="content-card"
                    style="margin-top:1rem;background:rgba(37,99,235,0.06);border-color:rgba(37,99,235,0.15);">
                    <h3
                        style="font-size:0.85rem;font-weight:700;color:var(--blue-400);margin-bottom:0.4rem;display:flex;align-items:center;gap:0.4rem;">
            ...
            <div class="chat-widget" style="margin-top:1.2rem;">
                ...
                <div id="chat-messages" class="chat-body">
                    ...
                    <div class="chat-row">
                        ...
                        <div class="chat-bubble bot-bubble">Halo! Saya PRIMM Bot 👋 Sekarang coba jalankan query-nya. Setelah melihat hasilnya, apakah sesuai dengan prediksimu? Kalau ada yang membingungkan, tanyakan ke saya!</div>
                    </div>
                    @else
    ...
    {{-- Nav bar --}}
    <div class="run-nav">
        @php $predictActivity = $chapter->activities->where('stage', 'predict')->first(); @endphp
        @if($predictActivity)
            <a href="{{ route('learning.activity', [$chapter, $predictActivity]) }}" class="nav-btn nav-btn-prev">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (btnRun && sqlEditor) {
                btnRun.addEventListener('click', async function() {
                    ...
                    if (!query) {
                        sqlOutput.innerHTML =
                            '<span style="color:#f38ba8;">Query tidak boleh kosong.</span>';
                        return;
                    }
                ...
                btnSubmit.addEventListener('click', async function() {
                    ...
                    try {
                        ...
                        if (data.success && data.is_correct) {
                            ...
                            addChat('assistant', '⚠️ ' + data.feedback + ' Perbaiki jawabanmu.');
