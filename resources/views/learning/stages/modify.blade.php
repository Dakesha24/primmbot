...
@section('content')
    <style>
        ...
        .mod-right::-webkit-scrollbar { width: 7px; }
        .mod-left::-webkit-scrollbar-track,
        .mod-right::-webkit-scrollbar-track { background: rgba(255,255,255,0.03); }
        .mod-left::-webkit-scrollbar-thumb,
        .mod-right::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.18); border-radius: 6px; }
        .mod-left::-webkit-scrollbar-thumb:hover,
        .mod-right::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.32); }
        ...
        .mod-nav .nav-btn-prev {
            color: #94a3b8;
            border-color: rgba(255,255,255,0.18);
        }

        .mod-nav .nav-btn-prev:hover {
            color: #e2e8f0;
            background: rgba(255,255,255,0.06);
            border-color: rgba(255,255,255,0.28);
        }

        .mod-nav .nav-btn-next {
            color: #cbd5e1;
        ...
        @media (max-width: 900px) {
            .mod-layout { grid-template-columns: 1fr; overflow-y: auto; }
            .mod-left { border-right: none; border-bottom: 1px solid rgba(255,255,255,0.06); }
        }
    ...
    <div class="mod-header">
        <h1 style="font-size:1.05rem;font-weight:800;color:#fff;letter-spacing:0.06em;text-transform:uppercase;">Modify</h1>
    ...
    <div id="help-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:1000;align-items:center;justify-content:center;" onclick="if(event.target===this)this.style.display='none'">
        <div style="background:#0d1f3a;border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:1.5rem 1.75rem;max-width:460px;width:90%;position:relative;">
            ...
            <h3 style="font-size:0.95rem;font-weight:700;color:#fff;margin-bottom:0.75rem;display:flex;align-items:center;gap:0.5rem;">
                <span style="background:rgba(37,99,235,0.2);color:var(--blue-400);padding:0.2rem 0.7rem;border-radius:6px;font-size:0.8rem;">MODIFY</span>
                Petunjuk Tahap
            </h3>
            <ul style="font-size:0.85rem;color:#94a3b8;line-height:1.8;padding-left:1.2rem;">
                <li>Baca perintah yang diberikan dengan teliti</li>
                <li>Modifikasi kode SQL yang sudah tersedia di editor sesuai perintah</li>
                <li>Klik <strong style="color:#cbd5e1;">Run ▶</strong> untuk melihat apakah hasilnya sudah sesuai</li>
                <li>Jawab pertanyaan penjelasan, lalu klik <strong style="color:#cbd5e1;">Cek</strong> dan <strong style="color:#cbd5e1;">Submit</strong></li>
            </ul>
        </div>
    </div>
            ...
            <div class="content-card">
                ...
                <div style="border-top:1px solid rgba(255,255,255,0.06);padding-top:1.2rem;">
                    ...
                    <div style="display:flex;align-items:center;gap:0.75rem;margin-top:1rem;">
                        ...
                        <button id="btn-submit" {{ ($submission && ($submission->is_correct || ($submission->score !== null && $submission->score >= 70))) ? '' : 'disabled' }}>Submit</button>
                    </div>
            ...
            <div class="chat-widget" style="margin-top:1.2rem;">
                ...
                <div id="chat-messages" class="chat-body">
                    <div class="chat-row">
                        ...
                        <div class="chat-bubble bot-bubble">Halo! Saya PRIMM Bot 👋 Di tahap Modify, kamu perlu memodifikasi query yang sudah ada. Kalau bingung di bagian mana yang harus diubah, tanyakan ke saya!</div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const chatApiKey    = 'primm_api_' + activityId;
            ...
            /* ── Mutable state ── */
            let chatHistory    = JSON.parse(localStorage.getItem(chatApiKey) || '[]');
            let chatVisLog     = JSON.parse(localStorage.getItem(chatVisKey) || '[]');
            let submitUnlocked = {{ ($submission && $submission->score !== null && $submission->score >= 70) ? 'true' : 'false' }};
            ...
            function saveChat() {
                ...
                localStorage.setItem(chatApiKey, JSON.stringify(chatHistory));
            }
            ...
            function addChat(role, msg) {
                if (!chatBox) return;
                removeTyping();
                chatBox.appendChild(role === 'user' ? makeUserRow(msg) : makeBotRow(msg));
            ...
            btnCek.addEventListener('click', async function() {
                ...
                try {
                    ...
                    const data = await res.json();
                    if (data.success) {
                        const isGood = data.score !== null && data.score !== undefined && data.score >= 70;
                        const scoreInfo = (data.score !== null && data.score !== undefined) ? ' (Skor: ' + data.score + '/100)' : '';
                        addChat('assistant', data.feedback + scoreInfo);
                        if (!submitUnlocked && isGood) { submitUnlocked = true; setSubmitLocked(false); }
                    } else {
            ...
            btnSubmit.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (data.success && data.is_correct) {
                        addChat('assistant', '✅ ' + data.feedback + ' (Skor: ' + data.score + '/100)');
