<x-layouts.admin title="Edit Aktivitas — Modify">
    <x-slot:styles>
        ...
        <style>
            ...
            .stage-pill { font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 4px; background: #fce7f3; color: #9d174d; }
            ...
            .expected-output-preview { margin-top: 12px; border: 1px solid #e4e8f1; border-radius: 6px; overflow: hidden; }
            .expected-output-preview .preview-header { display: flex; align-items: center; gap: 7px; padding: 8px 14px; font-size: 12px; font-weight: 600; color: #3b5bdb; background: #f0f4ff; border-bottom: 1px solid #e4e8f1; }
            .expected-output-preview .preview-scroll { overflow-x: auto; max-height: 220px; }
            ...
            .level-info-badge { display: inline-flex; align-items: center; gap: 6px; padding: 5px 14px; border-radius: 5px; background: #fce7f3; color: #9d174d; font-size: 12px; font-weight: 700; }
            .level-info-badge .level-dot { width: 6px; height: 6px; border-radius: 50%; background: #9d174d; opacity: 0.6; }
            ...
            .btn-level-help:hover { background: #fce7f3; border-color: #9d174d; color: #9d174d; }
    ...
    <div class="breadcrumb">
        ...
        <span>›</span>
        <span>Edit Aktivitas — Modify</span>
    </div>
        ...
        <div class="level-modal-box">
            ...
            <div class="level-item">
                ...
                <div class="level-desc">
                    <strong>Mudah</strong>
                    Modifikasi ringan pada query yang sudah ada. Contoh: mengubah kolom yang ditampilkan, menambah kondisi WHERE sederhana, atau mengubah urutan ORDER BY.
                </div>
            </div>
            <div class="level-item">
                ...
                <div class="level-desc">
                    ...
                    Modifikasi yang memerlukan pemahaman struktur query. Contoh: mengubah tipe JOIN (INNER → LEFT), menambah GROUP BY, atau menambah fungsi agregasi.
                </div>
            </div>
            <div class="level-item">
                ...
                <div class="level-desc">
                    ...
                    Modifikasi kompleks yang menguji pemahaman mendalam. Contoh: mengubah logika kondisi JOIN, menggabungkan beberapa tabel, atau menggunakan sub-query.
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="mod-grid">
            ...
            <div class="db-panel">
                <div class="db-panel-head">
                    ...
                    <span class="stage-pill">MODIFY</span>
                </div>
                <div class="db-panel-inner">
                    ...
                    <div class="db-hint">Database wajib dipilih untuk tahap Modify.</div>
                    <div id="db-dynamic">
            ...
            <div class="form-panel">
                ...
                <div class="level-info-row">
                    ...
                    <span class="level-info-note">Level ditetapkan otomatis berdasarkan urutan soal</span>
                    <button type="button" class="btn-level-help" onclick="openLevelModal()" title="Panduan level Modify">?</button>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="question_text" rows="2" required
                        placeholder="Contoh: Modifikasi query berikut agar menampilkan hanya buku dengan harga di atas 50.000.">{{ old('question_text', $activity->question_text) }}</textarea>
                    <div class="field-hint" style="margin-top:5px;">Instruksi modifikasi yang harus dilakukan siswa.</div>
                </div>

                <hr class="section-divider">

                {{-- Kode Default Editor --}}
                <div class="section-title">Kode Default Editor</div>
                <div class="field-hint" style="margin-bottom:12px;">Kode SQL awal yang diberikan ke siswa untuk dimodifikasi.</div>
                <div class="admin-editor-wrap">
                    <div class="admin-editor-bar">
                        <span class="admin-editor-bar-label">SQL Awal</span>
                    ...
                    <textarea name="editor_default_code" id="sqlInput" class="admin-editor-code" rows="6"
                        placeholder="SELECT *&#10;FROM buku&#10;JOIN penerbit ON buku.id_penerbit = penerbit.id_penerbit;">{{ old('editor_default_code', $activity->editor_default_code) }}</textarea>
                ...
                <div class="field-hint" style="margin-bottom:12px;">SQL yang sudah dimodifikasi dengan benar. Klik <strong>Jalankan & Simpan ▶</strong> untuk memperbarui expected output.</div>
                <div class="admin-editor-wrap">
                    <div class="admin-editor-bar">
                    ...
                    <textarea name="reference_sql" id="sqlRef" class="admin-editor-code" rows="6"
                        placeholder="SELECT *&#10;FROM buku&#10;JOIN penerbit ON buku.id_penerbit = penerbit.id_penerbit&#10;WHERE buku.harga > 50000;">{{ old('reference_sql', $activity->reference_sql) }}</textarea>
                    <div id="ref-result-wrap"></div>
                </div>
                <input type="hidden" name="expected_output" id="expectedOutputInput"
                    value="{{ old('expected_output', is_array($activity->expected_output) ? json_encode($activity->expected_output) : $activity->expected_output) }}">

                {{-- Auto-render expected output jika sudah ada --}}
                @php $savedOutput = is_array($activity->expected_output) ? $activity->expected_output : (json_decode($activity->expected_output ?? '[]', true) ?? []); @endphp
                @if(count($savedOutput) > 0)
                    <div class="expected-output-preview" id="ref-result-wrap">
                        <div class="preview-header">
                            ...
                            Expected output tersimpan — {{ count($savedOutput) }} baris
                        </div>
                        <div class="preview-scroll">
                            <table class="result-table">
                                <thead><tr>@foreach(array_keys($savedOutput[0]) as $col)<th>{{ $col }}</th>@endforeach</tr></thead>
                                <tbody>
                                    @foreach($savedOutput as $row)
                                        <tr>@foreach($row as $val)<td>{{ $val ?? 'NULL' }}</td>@endforeach</tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="field-hint" style="margin-bottom:12px;">Pertanyaan yang meminta siswa menjelaskan apa yang diubah dan mengapa.</div>
            ...
            async function runSql() {
                ...
                try {
                    ...
                    if (data.success && data.type === 'select') {
                        ...
                        else { html += '<div style="padding:14px;font-size:12px;color:#94a3b8;">Tidak ada kolom yang dikembalikan.</div>'; }
                ...
                } catch (e) { resultWrap.innerHTML = `<div class="sql-result-box"><div class="sql-result-header error">Koneksi gagal</div><div class="sql-error-msg">Tidak dapat terhubung ke server.</div></div>`; }
                finally { btn.disabled = false; btn.innerHTML = `<svg width="10" height="10" fill="currentColor" viewBox="0 0 24 24"><polygon points="5,3 19,12 5,21"/></svg> Jalankan ▶`; }
            }
